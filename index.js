...
async function run() {
  try {
    ...
    const collegeCollection=client.db('Kingstar').collection('colleegs');
    ...
    app.get('/mycollege/:email',async(req,res)=>{
        ...
        const result=await collegeCollection.find({postedBy:req.params.email}).toArray();
        res.send(result);
      });
...
app.get('/', (req, res) => {
  res.send('Hello World!I am honuman and so on')
})
